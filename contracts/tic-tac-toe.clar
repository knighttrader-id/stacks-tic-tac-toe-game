...
(define-constant ERR_INVALID_BET_INCREMENT (err u108))
...
;; Betting constants
(define-constant MIN_BET_AMOUNT u1000) ;; 0.001 STX in microSTX
(define-constant BET_INCREMENT u1000)  ;; 0.001 STX increments
...
(define-public (create-game (bet-amount uint) (move-index uint) (move uint))
    ...
    ;; Input validation
    (asserts! (validate-bet-amount bet-amount) ERR_MIN_BET_AMOUNT)
...
(define-read-only (get-betting-info)
    (ok {
        min-bet-amount: MIN_BET_AMOUNT,
        bet-increment: BET_INCREMENT,
        min-bet-stx: "0.001"
    })
)
...
;; Validate bet amount meets minimum and increment requirements
(define-private (validate-bet-amount (amount uint))
    (and 
        (>= amount MIN_BET_AMOUNT)
        (is-eq (mod amount BET_INCREMENT) u0)
    )
)
